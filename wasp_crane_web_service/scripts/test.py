...
class CraneController:
    def __init__(self) -> None:
        ...
        self.width = None
        self.default_motion_speed = rospy.get_param("/default_motion_speed", default=100)
        self.slow_motion_speed = rospy.get_param("/slow_motion_speed", default=80)
        self.fast_motion_speed = rospy.get_param("/fast_motion_speed", default=120)
        self.default_extrusion_speed = rospy.get_param("/default_extrusion_speed", default=100)
        self.slow_extrusion_speed = rospy.get_param("/slow_extrusion_speed", default=80)
        self.fast_extrusion_speed = rospy.get_param("/fast_extrusion_speed", default=120)
        self.default_width = rospy.get_param("/default_width", default=1)
        self.width_threshold = rospy.get_param("/width_threshold", default=0.1)
    ...
    def retreive_printer_status(self) -> dict:
        ...
        response = requests.get(
            "http://192.168.0.129/api/printer",
    ...
    def adjust_width(self, motion_speed: int, extrusion_speed: int) -> None:
        try:
            ...
            response_flr = requests.post(
                "http://192.168.0.129/api/printer/tool",
                headers=self.headers,
